...
export const googleSignIn = (formData, navigate) => async(dispatch) => {
    ...
    try {
        //login the user
        const response = await api.googleSignIn(formData);
...
export const referral = (formData, navigate) => async() => {
    const toastId = toast.loading("Loading..")
    try {
        //login the user
        const response = await api.referral(formData);
        console.log(response);
        ...
        toast.success("Referral Successfully Submitted 🎉");
        navigate('/referralView');
    }
    catch (error) {
        console.log(error);
        toast.error(error.response.data.message);
